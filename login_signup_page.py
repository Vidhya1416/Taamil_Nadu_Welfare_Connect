...
def show_login_signup(role, root, back_callback, show_login_callback, show_signup_callback):
    ...
    login_button = tk.Button(root, text="Login", command=lambda: show_login_callback(role))
    login_button.pack(pady=10)

    if role.lower() != "admin":  # Check if the role is not admin
        signup_button = tk.Button(root, text="Signup", command=lambda: show_signup_callback(role, root, back_callback))
